...
- `nx` and `ny` are the number of times the scene will be cloned by the grid cloner in each
direction along the x and y axis (e.g., setting `nx = 1` and `ny = 1` will generate a grid
of 3 x 3 clones of the original scene), whereas `dx` and `dy` will be distance at which each
new clone will be generated (along the axis).

- `nz` is the number of times the scene will be cloned in the vertical direction. Unlike
